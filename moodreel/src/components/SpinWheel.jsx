  ...
  const drawWheel = useCallback(
    currentRotation => {
      ...
      wheelMovies.forEach((movie, i) => {
        ...
        ctx.fillText(
          title + (title.length < (movie.title || movie.name || '').length ? '...' : ''),
          radius - 15,
          5
        );
        ctx.restore();
  ...
  const spin = useCallback(() => {
    ...
    const animate = () => {
      ...
      if (progress < 1) {
        ...
        const winnerIndex =
          Math.floor((360 - normalizedRotation) / segmentAngle) % wheelMovies.length;
    ...
    <div className="spin-wheel-overlay" onClick={onClose}>
      <div className="spin-wheel-modal" onClick={e => e.stopPropagation()}>
        ...
        <div className="spin-wheel-container">
          <canvas ref={canvasRef} width={350} height={350} className="spin-wheel-canvas" />
        </div>
