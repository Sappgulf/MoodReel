    ...
    <div className={`watchlist-page ${!hasSavedItems ? 'watchlist-page-empty' : ''}`}>
      ...
      {hasSavedItems && (
        <div className="watchlist-layout-toggle" role="group" aria-label="Watchlist layout">
          ...
          >
            <LayoutGrid size={16} aria-hidden="true" /> Poster Board
          </button>
          <button
            type="button"
          ...
          >
            <List size={16} aria-hidden="true" /> Film Log
          </button>
        </div>
      )}
      ...
      {activeTab === 'watchlist' && watchlist.length > 0 && (
        <div className="watched-filter">
          ...
          >
            <Check size={14} aria-hidden="true" /> Watched ({watchedCount})
          </button>
          <button
            className={showWatched === 'unwatched' ? 'active' : ''}
            onClick={() => setShowWatched('unwatched')}
          >
            <Eye size={14} aria-hidden="true" /> To Watch ({watchlist.length - watchedCount})
          </button>
        ...
        <div className="personalized-recs">
          <div className="recs-header">
            <h3>
              {recsBasedOn ? (
                <>
                  <Sparkles size={16} aria-hidden="true" /> Because you saved{' '}
                  <em>{recsBasedOn.title}</em>
                </>
              ) : (
                <>
      ...
      {showMatchmaker && (
        <div className="matchmaker-panel">
          <h4>
            <Users size={16} aria-hidden="true" /> Movie Matchmaker
          </h4>
      ...
      {sortedList.length === 0 ? (
        <EmptyState
          icon={activeTab === 'favorites' ? Heart : watchlist.length === 0 ? Clapperboard : Search}
          title={
            activeTab === 'favorites'
              ? 'No favorites yet'
              : watchlist.length === 0
