...
export function useTasteProfile() {
    const [profile, setProfile] = useState(() => readStoredJSON(STORAGE_KEY, { liked: [], disliked: [] }));
    ...
    const isLiked = useCallback((itemId, mediaType) => {
        return profile.liked.includes(getItemKey(itemId, mediaType));
    }, [profile.liked]);

    const isDisliked = useCallback((itemId, mediaType) => {
        return profile.disliked.includes(getItemKey(itemId, mediaType));
    }, [profile.disliked]);

    const statusFor = useCallback((itemId, mediaType) => {
