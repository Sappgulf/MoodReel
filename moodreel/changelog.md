...
- Fixed: Restored `Tonight.jsx` after a bad merge left invalid JSX on `main` (blocked build/format checks).
- Fixed: Vitest now scopes unit tests to `src/**` and excludes Playwright `e2e/` specs.
- Changed: Prettier formatting pass across the web app for green `format:check` in CI.
